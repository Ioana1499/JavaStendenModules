 ...
 *
 * This class is the base business for all models that store entities.
 * @param <T> the business type parameter
 */
public abstract class NonPersistingRepository<T extends Storable> implements Repository<T>
{
    ...
    /**
     * Instantiates a new Storage business.
     *
        ...
        repository.stream().filter( e -> e.getId() == entity.getId() ).findFirst() // Find object with same id
                .ifPresent( model -> { // If so throw exception.
                    throw new IllegalArgumentException( String.format(
                            "Cannot add a new %1$s business to the %1$s repository because the id already exists.",
    ...
    public T getById( int id )
    {
        return repository.stream().filter( item -> item.getId() == id ).findFirst()
                .orElseThrow( () -> new NoSuchElementException( String.format( "No business found with an id of:%d", id ) ) );
    }
    ...
    /**
     * Has business where condition is true.
     *
     * @param condition the condition
     * @return the boolean
     */
